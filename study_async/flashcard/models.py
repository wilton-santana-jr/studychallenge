...
class Desafio(models.Model):
    STATUS_CHOICES = (('A', 'Aberto'), ('F', 'Finalizado'))
    user = models.ForeignKey(User, on_delete=models.DO_NOTHING)
    ...
    status = models.CharField(
        ...
        #max_length=1, choices=Desafio.STATUS_CHOICES
    )

    @property
    def css_dificuldade(self):
        if self.dificuldade == 'F':
            return 'desafio-dificuldade-facil'
        elif self.dificuldade == 'M':
            return 'desafio-dificuldade-medio'
        elif self.dificuldade == 'D':
            return 'desafio-dificuldade-dificil'
    
    @property
    def descricao_dificuldade(self):
        if self.dificuldade == 'F':
            return 'Fácil'
        elif self.dificuldade == 'M':
            return 'Médio'
